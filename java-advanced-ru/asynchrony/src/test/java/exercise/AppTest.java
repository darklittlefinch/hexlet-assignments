...
class AppTest {
    ...
    // BEGIN
    @Test
    public void testGetDirectorySize() throws ExecutionException, InterruptedException {
        var actual = App.getDirectorySize("src/test/resources").get();

        var expected = App.parse("src/test/resources/file1.txt").toFile().length()
                + App.parse("src/test/resources/file2.txt").toFile().length();
